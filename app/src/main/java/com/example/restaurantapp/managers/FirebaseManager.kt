package com.example.restaurantapp.managers
...
class FirebaseManager private constructor(){
    ...
    fun getRestaurantDetails(username: String): LiveData<Restaurant?> {
        val userLiveData = MutableLiveData<Restaurant?>()
        userCollection.whereEqualTo("username", username).get()
            .addOnCompleteListener { task ->
                if(task.isSuccessful) {
                    val res = task.result?.documents?.firstOrNull()
                    if(res != null) {
                        val user = res.toObject<Restaurant>()
    ...
    fun addRestaurant(restaurant: Restaurant, callback: (Boolean) -> Unit) {
    ...
    fun getCurrentRestaurant(): FirebaseUser? {
